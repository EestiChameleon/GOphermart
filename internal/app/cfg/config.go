...
type Config struct {
	...
	DatabaseURI    string `env:"DATABASE_URI" envDefault:"postgresql://localhost:5432/yandex_practicum_db?sslmode=disable"` // Строка с адресом подключения к БД
	AccrualSysAddr string `env:"ACCRUAL_SYSTEM_ADDRESS" envDefault:"http://localhost:666"`                                  // Адрес системы расчёта начислений «http://server:port»
}
